    ...
    fn choose(&mut self, b: &Board) -> Option<Move> {
        ...
        for (mv, _) in ranked.into_iter().take(k) {
            ...
            for &seed in &stream_seeds {
                ...
                let refills: Vec<u64> = (0..mv.path.len() - 1).map(|_| stream.rnd13()).collect();
...
pub fn c1() -> CalVal {
    CalVal {
        beta0: -488.697,
        betas: [
            7.8070, -2.7676, 53.8095, 0.1974, 0.0000, 24.6987, -6.7524, 217.6770, -20.1253,
            -23.0711, -23.7403, -51.1202, -73.0807, -37.3657, 17.7916, 12.0566, -17.4748, -1.4949,
            24.8876, 39.7930, 14.2424, -13.3476, -29.1856, -1.6993, 0.0513, 1.1128, -0.6707,
            0.0154, 14.2533, 192.0977,
        ],
    }
}
